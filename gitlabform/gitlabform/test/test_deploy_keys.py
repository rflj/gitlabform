from gitlabform.gitlabform.test import (
    run_gitlabform,
)


class TestDeployKeys:
    def test__deploy_key_to_all_projects(self, gitlab, group, project, other_project):
        deploy_key_to_all_projects = """
        projects_and_groups:
          "*":
            deploy_keys:
              foobar:
                key: ssh-rsa 
                title: foobar_key
                can_push: false
        """
        run_gitlabform(deploy_key_to_all_projects, group)

        deploy_keys1 = gitlab.get_deploy_keys(f"{group}/{project}")
        assert len(deploy_keys1) == 1

        deploy_keys2 = gitlab.get_deploy_keys(f"{group}/{other_project}")
        assert len(deploy_keys2) == 1

    def test__deploy_key_to_2_projects(self, gitlab, group, project, other_project):
        deploy_key_to_2_projects = f"""
            projects_and_groups:
              {project}:
                deploy_keys:
                  the_key:
                    key: ssh-rsa 
                    title: the_key
                    can_push: false
              {other_project}:
                 deploy_keys:
                   the_key_under_a_different_name:
                     key: ssh-rsa 
                     title: the_key_under_a_different_name
                     can_push: false
            """
        run_gitlabform(deploy_key_to_2_projects, group)

        deploy_keys1 = gitlab.get_deploy_keys(f"{group}/{project}")
        assert len(deploy_keys1) == 1

        deploy_keys2 = gitlab.get_deploy_keys(f"{group}/{other_project}")
        assert len(deploy_keys2) == 1
